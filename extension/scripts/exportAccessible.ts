...
${simulators.length ? `<aside aria-label="Official OSTEP prediction tools"><h3>Official OSTEP prediction tools</h3><p>Read the named chapter and the tool's README, run without <code>-c</code>, record a prediction, then add <code>-c</code> only to check and explain your work.</p>${simulators.map((simulator) => `<section><h4>${escape(simulator.chapter)}: ${escape(simulator.title)}</h4><p>${escape(simulator.purpose)}</p><p><strong>Prepare:</strong> ${escape(simulator.priorKnowledge)}</p><p><strong>Predict:</strong> ${escape(simulator.predict)}</p><p><strong>Run from:</strong> <code>official/${escape(simulator.directory)}</code></p><p><strong>Prediction command:</strong> <code>python3 ${escape(simulatorArguments(simulator, 'practice').join(' '))}</code></p><p><strong>Explain after checking:</strong> ${escape(simulator.explain)}</p></section>`).join('')}</aside>` : ''}
  ...
  <details><summary>Chapter and lecture alignment</summary><p>${escape(module.sourceBasis)}</p></details>
...
<body id="top"><a class="skip" href="#main">Skip to lessons</a><header><p class="eyebrow">Fall 2026 · Accessible text alternative</p><h1>${escape(COURSE.title)}</h1><p>These explanations and formative questions supplement the lecture sources. <a href="${escape(COURSE.canvasUrl)}">Canvas course 552201</a> is authoritative for schedule changes, assignments, submissions, and grades.</p><p>The modules provide 29 exact official OSTEP chapter links, 104 explained readiness questions, and 15 mapped simulator activities. The available nine instructor decks were individually compared with their concurrency and persistence chapters; missing virtualization decks are explicitly disclosed rather than inferred.</p><p><a href="${escape(OSTEP_HOMEWORK_PAGE)}">Official OSTEP simulator documentation</a>; extension-tested source revision: <code>${escape(OSTEP_HOMEWORK_COMMIT)}</code>. The source is fetched after consent and is not redistributed in this lesson package.</p><nav aria-label="Module list"><h2>Modules</h2><ol>${navigation}</ol></nav></header><main id="main"><section aria-labelledby="course-plan-title"><h2 id="course-plan-title">Dated OSTEP preparation plan</h2><p>The meeting dates, time, and room are verified. Topics and readings are the current preparation plan; Canvas announcements control changes and all assessed-work dates.</p><table><caption>Fall 2026 Monday/Wednesday reading map</caption><thead><tr><th>Meeting</th><th>Date</th><th>Day</th><th>Module</th><th>Planned focus</th><th>Read before class</th></tr></thead><tbody>${scheduleRows}</tbody></table></section>${content}</main><footer><p>Prepared for ${escape(COURSE.title)}. Last content build: 2026-08-20.</p></footer></body></html>`;

async function main(): Promise<void> {
  await mkdir(destination, { recursive: true });
  await writeFile(resolve(destination, 'CIS450_ECE478_Fall2026_Accessible_Lessons.html'), html, 'utf8');
  await writeFile(resolve(destination, 'module-map.csv'), ['module,unit,title,reading_chapters,reading_urls,official_simulators,hands_on,alignment_basis', ...MODULES.map((module) => [module.number, module.unit, module.title, module.readings.map((reading) => `${reading.chapter}: ${reading.title}`).join(' | '), module.readings.map((reading) => reading.url).join(' | '), OSTEP_SIMULATORS.filter((simulator) => simulator.moduleNumber === module.number).map((simulator) => `${simulator.chapter}: ${simulator.title}`).join(' | '), module.handsOn, module.sourceBasis].map((value) => `"${String(value).replaceAll('"', '""')}"`).join(','))].join('\n') + '\n', 'utf8');
}
