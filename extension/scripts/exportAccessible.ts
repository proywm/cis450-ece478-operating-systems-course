...
const scheduleRows = fall2026Schedule().map((meeting) => `<tr><td>${meeting.number}</td><td>${escape(meeting.date)}</td><td>${escape(meeting.day)}</td><td>${meeting.moduleNumbers.map((number) => `<a href="${moduleFile(number)}">Module ${number}</a>`).join(', ')}</td><td>${escape(meeting.topic)}</td><td>${escape(meeting.prepare)}</td></tr>`).join('\n');
