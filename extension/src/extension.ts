...
import { buildCalendar, FALL_2026_EXAMS, fall2026Schedule, tutorReply } from './core.js';
...
export function activate(context: vscode.ExtensionContext): SystemStudioExtensionApi {
  ...
  void promptForOrbitSetupOnFirstRun(context);
  return {
    integrationStatus: () => ({
  ...
  getChildren(element?: TreeNode): TreeNode[] {
    ...
    if (element.kind === 'start') {
      return [
        ...
        action('3. Complete the ungraded pre-test', 'systemstudioOs.openPretest', 'action', '0 points · unaided baseline · submit in Canvas'),
    ...
    if (element.kind === 'support') {
      return [
        ...
        action('Open dated OSTEP course plan', 'systemstudioOs.openLearningHub', 'action', '26 classes · Oct 14 midterm · Dec 14 final'),
        action('Export Fall 2026 calendar', 'systemstudioOs.exportCalendar', 'action', 'classes · midterm · final · readings'),
        action('Open ungraded beginning-of-course pre-test', 'systemstudioOs.openPretest', 'action', '0 points · complete without AI help'),
...
class LearningHub {
    ...
    if (!this.panel) {
      ...
      this.panel.webview.onDidReceiveMessage(async (message: Record<string, unknown>) => {
        if (message.type === 'tutor') {
          ...
          const allowed = new Set(['systemstudioOs.openCanvas', 'systemstudioOs.openSyllabus', 'systemstudioOs.openPretest', 'systemstudioOs.openAccessibleLessons', 'systemstudioOs.setupCourseEnvironment', 'systemstudioOs.openAiTutor', 'systemstudioOs.reportIssue', 'systemstudioOs.checkEnvironment', 'systemstudioOs.createLabWorkspace', 'systemstudioOs.runCourseworkPreflight', 'systemstudioOs.openPortableSetup', 'systemstudioOs.reopenInCourseContainer', 'systemstudioOs.runCurrentC', 'systemstudioOs.exportCalendar', 'systemstudioOs.configureCanvas', 'systemstudioOs.importCanvasCalendar', 'systemstudioOs.prepareXv6', 'systemstudioOs.verifyXv6', 'systemstudioOs.openXv6Guide', 'systemstudioOs.prepareOstepSimulators', 'systemstudioOs.openOstepSimulatorGuide']);
          ...
          if (decision === 'Reset Local Data') void this.panel?.webview.postMessage({ type: 'resetLocalDataConfirmed' });
  ...
  showOfflineHelp(starterPrompt?: string): void {
    this.show('help');
    if (starterPrompt) void this.panel?.webview.postMessage({ type: 'offlineStarter', text: starterPrompt });
  }
}

async function openBundledHtml(context: vscode.ExtensionContext, folder: 'syllabus' | 'lessons' | 'diagnostics', filename: string): Promise<void> {
...
async function configureAiAssistance(
  ...
  if (selection.preference === 'offline') {
    ...
    await vscode.window.showInformationMessage('Private offline Orbit support is ready. It can route setup errors and OS questions without sending data to an AI service.');
    return 'offline';
  }
...
async function prepareOstepSimulatorWorkspace(context: vscode.ExtensionContext): Promise<void> {
  ...
  try {
    ...
    if (choice === 'Run a simulator') await runOstepSimulator(context);
	    ...
	    const actionable = diagnostic.split(/\r?\n/).map((line) => line.trim()).find(Boolean)?.slice(0, 360)
	      ?? 'The simulator setup command stopped without a diagnostic line.';
	    output.appendLine(`FAILED: ${diagnostic}`);
	    output.appendLine(`A partial directory may remain at ${root.fsPath}; it was not deleted automatically.`);
	    void vscode.window.showErrorMessage(`OSTEP simulator setup stopped. First error: ${actionable} Next step: leave the partial folder in place, run the environment check, and retry in a new destination. No existing folder was overwritten or deleted.`);
  }
}
...
async function runOstepSimulator(context: vscode.ExtensionContext, requestedId?: string, requestedMode?: string): Promise<{ id: string; mode: OstepSimulatorMode; route: 'native' | 'docker' } | undefined> {
  ...
  try {
    ...
    return { id: simulator.id, mode, route: route.value };
	    ...
	    const diagnostic = [details.stderr, details.stdout, details.message ?? String(error)].filter(Boolean).join('\n');
	    const actionable = diagnostic.split(/\r?\n/).map((line) => line.trim()).find(Boolean)?.slice(0, 360)
	      ?? 'The simulator stopped without a diagnostic line.';
	    output.appendLine(`FAILED: ${details.message ?? String(error)}`);
	    output.appendLine('Keep this first error and run the environment check before reinstalling anything.');
	    void vscode.window.showErrorMessage(`The OSTEP simulator stopped. First error: ${actionable} Next step: run the course-environment check, then retry. Detailed simulator output remains available for advanced troubleshooting.`);
    return undefined;
  }
}
...
async function prepareXv6Workspace(): Promise<void> {
  ...
  try {
    ...
    if (choice === 'Open workspace') await vscode.commands.executeCommand('vscode.openFolder', root, { forceNewWindow: true });
	    ...
	    const diagnostic = error instanceof Error ? error.message : String(error);
	    const actionable = diagnostic.split(/\r?\n/).map((line) => line.trim()).find(Boolean)?.slice(0, 360)
	      ?? 'The xv6 setup command stopped without a diagnostic line.';
	    output.appendLine(`FAILED: ${diagnostic}`);
	    output.appendLine(`A partial directory may remain at ${root.fsPath}; it was not deleted automatically.`);
	    void vscode.window.showErrorMessage(`xv6 setup stopped. First error: ${actionable} Next step: leave the partial folder in place, run the environment check, and retry in a new destination. No existing folder was overwritten or deleted.`);
  }
}
...
async function verifyXv6Workspace(requestedMode?: string): Promise<void> {
  ...
  try {
    ...
    if (details.stdout) output.append(details.stdout);
	    ...
	    const diagnostic = [details.stderr, details.stdout, details.message ?? String(error)].filter(Boolean).join('\n');
	    const actionable = diagnostic.split(/\r?\n/).map((line) => line.trim()).find(Boolean)?.slice(0, 360)
	      ?? 'The xv6 preflight stopped without a diagnostic line.';
	    void vscode.window.showErrorMessage(`${mode.toUpperCase()} preflight stopped. First error: ${actionable} Next step: inspect the named assertion, run the environment check, and retry. Detailed xv6 output remains available for advanced troubleshooting.`);
  }
}
...
async function setupCourseEnvironment(
  ...
  try {
    ...
    const diagnostic = [details.stderr, details.stdout, details.message ?? String(error)].filter(Boolean).join('\n');
    const actionable = diagnostic.split(/\r?\n/).map((line) => line.trim()).find((line) => line.length > 0)?.slice(0, 360)
      ?? 'The course environment command stopped without a diagnostic line.';
    const choice = await vscode.window.showErrorMessage(
      `Guided setup stopped. First error: ${actionable} Next step: retry once; if it repeats, open the setup guide or ask Orbit using this diagnostic. Detailed output remains available for advanced troubleshooting.`,
...
async function exportCalendar(): Promise<void> {
  ...
  void vscode.window.showInformationMessage('Exported 26 regular class meetings, the October 14 midterm, the December 14 final, and academic-calendar boundaries. Canvas remains authoritative for assessment details.');
}
...
const defaults={section:'home',subTabs:{},moduleStatus:{},confidence:{},grade:{participation:'',homework:'',programming:'',midterm:'',finalExam:''},gradeTarget:'83.34',learning:emptyLearning(),coursework:{},labSteps:{},simulationRuns:{},canvasEvents:[],walkthroughOpen:DATA.firstRun,walkthroughStep:0,companionEnabled:true,companionOpen:false};
let state=Object.assign({},defaults,vscode.getState()||{});state.subTabs=state.subTabs||{};state.moduleStatus=state.moduleStatus||{};state.confidence=state.confidence||{};state.grade=Object.assign({},defaults.grade,state.grade||{});state.learning=state.learning||emptyLearning();state.coursework=state.coursework||{};state.labSteps=state.labSteps||{};state.simulationRuns=state.simulationRuns||{};state.canvasEvents=state.canvasEvents||[];
...
const subtabActivators={};
function unitKey(number){const unit=DATA.modules.find(module=>module.number===number)?.unit||'other';return String(unit).toLowerCase().replace(/[^a-z0-9]+/g,'-')}
function installSubtabs(root,id,groups,anchor){const available=groups.filter(group=>group.nodes.length);if(available.length<2||!anchor)return;const tabs=document.createElement('div'),panels=document.createElement('div');tabs.className='subtabs';tabs.setAttribute('role','tablist');tabs.setAttribute('aria-label',available.map(group=>group.label).join(', ')+' sections');panels.className='subtab-panels';available.forEach((group,index)=>{const tab=document.createElement('button'),panel=document.createElement('section'),key=group.key;tab.type='button';tab.id=id+'-tab-'+key;tab.dataset.subtab=key;tab.setAttribute('role','tab');tab.setAttribute('aria-controls',id+'-panel-'+key);tab.setAttribute('aria-selected',String(index===0));tab.tabIndex=index===0?0:-1;tab.textContent=group.label;panel.id=id+'-panel-'+key;panel.className='subtab-panel';panel.dataset.subpanel=key;panel.setAttribute('role','tabpanel');panel.setAttribute('aria-labelledby',tab.id);panel.hidden=index!==0;group.nodes.forEach(node=>panel.append(node));tabs.append(tab);panels.append(panel)});anchor.replaceWith(tabs,panels);function activate(key,moveFocus=false){const buttons=[...tabs.querySelectorAll('[data-subtab]')],selected=buttons.some(button=>button.dataset.subtab===key)?key:buttons[0]?.dataset.subtab;buttons.forEach(button=>{const active=button.dataset.subtab===selected;button.setAttribute('aria-selected',String(active));button.tabIndex=active?0:-1;if(active&&moveFocus)button.focus()});panels.querySelectorAll('[data-subpanel]').forEach(panel=>panel.hidden=panel.dataset.subpanel!==selected);state.subTabs[id]=selected;persist()}tabs.onclick=event=>{const button=event.target.closest('[data-subtab]');if(button)activate(button.dataset.subtab)};tabs.onkeydown=event=>{if(!['ArrowLeft','ArrowRight','Home','End'].includes(event.key))return;const buttons=[...tabs.querySelectorAll('[data-subtab]')],index=buttons.indexOf(document.activeElement);if(index<0)return;event.preventDefault();const next=event.key==='Home'?0:event.key==='End'?buttons.length-1:(index+(event.key==='ArrowRight'?1:-1)+buttons.length)%buttons.length;activate(buttons[next].dataset.subtab,true)};subtabActivators[id]=activate;activate(state.subTabs[id]||available[0].key)}
function unitGroups(items,nodes){return ['Virtualization','Concurrency','Persistence'].map(label=>({key:label.toLowerCase(),label,nodes:nodes.filter((_,index)=>unitKey(items[index].moduleNumber??items[index].number)===label.toLowerCase())}))}
function enhanceModuleTasks(){document.querySelectorAll('#modules .module').forEach((card,index)=>{const content=card.querySelector('.module-content'),nodes=[...content.children],practiceStart=nodes.findIndex(node=>node.tagName==='H3'&&node.textContent.includes('Reading-aligned')),selfIndex=nodes.findIndex(node=>node.tagName==='FIELDSET'),simulationStart=nodes.findIndex(node=>node.tagName==='H3'&&node.textContent.includes('Official OSTEP')),handsIndex=nodes.findIndex(node=>node.classList?.contains('notice'));if(practiceStart<0||selfIndex<0||handsIndex<0)return;const applyIndexes=new Set();if(simulationStart>=0)for(let i=simulationStart;i<handsIndex;i++)applyIndexes.add(i);applyIndexes.add(handsIndex);const practiceIndexes=new Set();for(let i=practiceStart;i<selfIndex;i++)practiceIndexes.add(i);const marker=document.createElement('div');content.prepend(marker);installSubtabs(content,'module-'+(index+1)+'-tasks',[{key:'learn',label:'Learn',nodes:nodes.filter((_,i)=>!applyIndexes.has(i)&&!practiceIndexes.has(i)&&i!==selfIndex)},{key:'practice',label:'Practice 8',nodes:nodes.filter((_,i)=>practiceIndexes.has(i))},{key:'apply',label:'Apply',nodes:nodes.filter((_,i)=>applyIndexes.has(i))},{key:'reflect',label:'Self-check',nodes:selfIndex>=0?[nodes[selfIndex]]:[]}],marker)})}
function enhanceModules(){const root=document.getElementById('modules'),list=root.querySelector('.module-list');if(!list)return;enhanceModuleTasks();const cards=[...list.children],groups=['Virtualization','Concurrency','Persistence'].map(label=>({key:label.toLowerCase(),label,nodes:cards.filter((_,index)=>unitKey(DATA.modules[index].number)===label.toLowerCase())}));installSubtabs(root,'modules',groups,list)}
function enhanceLabs(){const root=document.getElementById('labs'),list=root.querySelector('.module-list'),pinned=root.querySelector(':scope > section.card');if(!list||!pinned)return;const marker=document.createElement('div');pinned.before(marker);const cards=[...list.children],groups=[{key:'xv6',label:'xv6 pathway',nodes:[pinned]},...unitGroups(DATA.labs,cards)];list.remove();installSubtabs(root,'labs',groups,marker)}
function enhanceSimulations(){const root=document.getElementById('simulations'),list=root.querySelector('.module-list');if(!list)return;installSubtabs(root,'simulations',unitGroups(DATA.simulations,[...list.children]),list)}
...
function enhanceHelp(){const root=document.getElementById('help'),sections=[...root.querySelectorAll(':scope > section')],boundary=root.querySelector(':scope > .notice');if(sections.length<6)return;const report=document.createElement('section');report.className='card';report.innerHTML='<h2>Report an extension problem</h2><p>Prepare a privacy-limited public GitHub draft with version and platform details. You review it before anything is submitted; files, code, grades, credentials, Canvas data, and logs are not attached.</p><button data-command="systemstudioOs.reportIssue">Prepare GitHub issue draft</button>';sections[0].after(report);bindCommands(report);const marker=document.createElement('div');sections[0].before(marker);installSubtabs(root,'help',[{key:'blocked',label:'Get unstuck',nodes:[sections[0],report]},{key:'coaching',label:'AI & offline help',nodes:sections.slice(1,4)},{key:'faq',label:'FAQ',nodes:[sections[4]]},{key:'before-class',label:'Ask before class',nodes:[sections[5],...(boundary?[boundary]:[])]}],marker)}
...
function renderSchedule(){const el=document.getElementById('schedule');const rows=DATA.schedule.map(m=>'<tr'+(m.kind==='assessment'?' class="assessment"':'')+'><td>'+esc(m.label)+'</td><td>'+esc(new Date(m.date+'T12:00:00').toLocaleDateString(undefined,{weekday:'short',month:'short',day:'numeric'}))+'</td><td><button class="quiet schedule-module" data-module-number="'+m.moduleNumbers[0]+'">'+m.moduleNumbers.map(n=>'M'+n).join(', ')+'</button></td><td>'+(m.kind==='assessment'?'<strong>'+esc(m.topic)+'</strong>':esc(m.topic))+'</td><td>'+esc(m.prepare)+'</td></tr>').join('');el.innerHTML='<p class="eyebrow">26 regular classes · 1 midterm · scheduled final</p><h1>Fall 2026 course plan and examinations</h1><div class="notice"><strong>Midterm:</strong> Wed Oct 14, '+esc(DATA.exams.midterm.time)+', '+esc(DATA.exams.midterm.room)+'; '+esc(DATA.exams.midterm.coverage)+'.<br><strong>Final:</strong> Mon Dec 14, '+esc(DATA.exams.final.time)+', '+esc(DATA.exams.final.room)+'; '+esc(DATA.exams.final.coverage)+'.</div><div class="notice warning"><strong>Check Canvas before each exam.</strong> The dates, class time, and room are recorded here; Canvas controls scope, format, allowed materials, accommodations, and later changes.</div><div class="actions"><button data-command="systemstudioOs.exportCalendar">Export classes and exams (.ics)</button><button data-command="systemstudioOs.openCanvas" class="secondary">Open Canvas course</button></div><table><caption>Monday/Wednesday class and assessment map</caption><thead><tr><th>Class / assessment</th><th>Date</th><th>Module</th><th>Focus</th><th>Prepare</th></tr></thead><tbody>'+rows+'</tbody></table>';el.onclick=e=>{const b=e.target.closest('.schedule-module');if(b)openModule(Number(b.dataset.moduleNumber))};bindCommands(el);}
...
enhanceModules=()=>{enhanceModulesBase()};
enhancePractice=()=>{const root=document.getElementById('practice');root.querySelectorAll('[data-confidence-q]').forEach(select=>{if(!select.querySelector('option[value=""]'))select.insertAdjacentHTML('afterbegin','<option value="">Choose confidence</option>');select.value=''});if(!root.dataset.confidenceGuard){root.dataset.confidenceGuard='true';root.addEventListener('click',event=>{const check=event.target.closest('.check');if(!check)return;const select=root.querySelector('[data-confidence-q="'+CSS.escape(check.dataset.q)+'"]');if(select&&!select.value){event.preventDefault();event.stopImmediatePropagation();const out=document.getElementById('practice-ex-'+check.dataset.q);if(out){out.hidden=false;out.textContent='Choose your confidence before requesting feedback.'}select.focus()}},true)}enhancePracticeBase()};
renderCoursework=()=>{renderCourseworkBase();addCourseworkExecutionControls();enhanceCoursework()};
renderHelp=()=>{renderHelpBase();enhanceHelp()};
openModule=n=>{show('modules');subtabActivators.modules?.(unitKey(n));const card=document.getElementById('module-'+n);if(card){const button=card.querySelector('.toggle'),content=card.querySelector('.module-content');content.hidden=false;button.setAttribute('aria-expanded','true');button.lastElementChild.textContent='−';card.scrollIntoView({behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'auto':'smooth',block:'start'});button.focus()}};
openLab=id=>{show('labs');const lab=DATA.labs.find(item=>item.id===id);if(lab)subtabActivators.labs?.(unitKey(lab.moduleNumber));const card=document.getElementById('lab-'+id);if(card){card.scrollIntoView({behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'auto':'smooth',block:'start'});card.querySelector('h2')?.focus()}};
show=id=>{showBase(id);const heading=document.getElementById(id)?.querySelector('h1');heading?.setAttribute('tabindex','-1');heading?.focus()};
