...
|---|---|---|
| Visible course map | Thirteen sidebar and in-hub modules grouped by virtualization, concurrency, and persistence | `courseData.test.ts`; webview parity test |
...
| FAQ/helper | Structured FAQ and deterministic offline helper with assessed-work refusal and attempt-first prompts | `core.test.ts`; `learning.test.ts` |
...
| Course operations | Verified meeting calendar, accessible syllabus, conservative staffing status, safe Canvas link configuration, reviewed ICS import, and pre-class draft handoff | calendar, learning, and release tests |
...
- The OS course has a verified historical x86 xv6 reference but no confirmed
  Fall 2026 xv6 revision, direct Canvas ID, assignment deadlines, or grading
  change. The extension labels the distinction and defers active facts to
  Canvas instead of fabricating them.
